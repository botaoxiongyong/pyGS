...
#print l[0][0]
finalresult = []
with open('/home/jiabo/google_drive/硕士资料/兰大/liuhao_result_900.txt', 'w') as file_:
    for i in np.arange(1799, 1800, step=1):
        singleresult =[]

        smaplenum = str(l[i][0])

        #print l[1][0]
        #smaplenum = str(l[10][0])                    #样品号
        ...
        #singleresult.append(smaplenum)
        #singleresult.extend(func(x_fit, result.params))
        item = list(func(x_fit, result.params))
        file_.write("%s\n" % smaplenum)
        file_.write("%s\n" % item)
        ...
        for i in np.arange(group):
            ...
            #singleresult.append('endmember'+str(i))
            #singleresult.extend(single(x_fit, para))
            file_.write("%s\n" % 'endmember'+str(i))
            file_.write("%s\n" % list(single(x_fit, para)))
        #print singleresult

        #finalresult.append([singleresult])

        print smaplenum
        #print singleresult[0]
